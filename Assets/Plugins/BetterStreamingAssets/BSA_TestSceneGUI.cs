...
namespace Better.StreamingAssets
{
    public class BSA_TestSceneGUI : MonoBehaviour
    {
        ...
        private delegate void TestResultDelegate(TimeSpan avgDuration, long avgBytesRead, long avgMemoryPeak,
            long maxMemoryPeak, string[] assetNames);
        ...
        void OnGUI()
        {
            using (new GUILayout.AreaScope(new Rect(0, 0, Screen.width, Screen.height)))
            {
                ...
                {
                    coroutineHost.StartCoroutine(TestAllCoroutine(m_selectedPaths.ToArray(), RepetitionCount,
                        m_readModes, m_testModes, m_results));
                }
        ...
        private IEnumerator TestAllCoroutine(IEnumerable<string> paths, int attempts, ReadMode readModes,
            TestType testTypes, List<TestInfo> results)
        {
            LogWorkProgress("starting...");

            string logPath = Path.Combine(Application.persistentDataPath,
                "BSA_test_" + DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss") + "_.csv");
                ...
                foreach (var path in paths)
                {
                    foreach (ReadMode readMode in Enum.GetValues(typeof(ReadMode)))
                    {
                        ...
                        foreach (TestType testType in Enum.GetValues(typeof(TestType)))
                        {
                            ...
                            yield return coroutineHost.StartCoroutine(ErrorCatchingCoroutine(TestHarness(readMode, path,
                                    testType, attempts,
                ...
                if (LogToFile)
                {
                    using (var writer = File.CreateText(logPath))
                    {
                        foreach (var result in results)
                        {
                            ...
                            writer.WriteLine("\"{0}\"\t{1}\t{2}\t{3}\t{4}\t\"{5}\"", result.path, result.readMode,
                                result.testType, result.duration, result.memoryPeak, errorMessage);
                        }
                    }
        ...
        private IEnumerator TestHarness(ReadMode readMode, string path, TestType testType, int attempts,
            TestResultDelegate callback)
        {
...
#pragma warning restore 0618 // Type or member is obsolete
            ...
            callback(new TimeSpan(stopwatch.ElapsedTicks / attempts), bytesRead / attempts, totalMemoryPeaks / attempts,
                maxMemoryPeak, assetNames);
        }
    }
}