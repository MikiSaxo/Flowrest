...
public class FirstMapManager : MonoBehaviour
{
    ...
    public static FirstMapManager Instance;
    ...
    private void InitObj(GameObject which, int i, int j, bool hasGroundManager)
    {
        ...
        if (hasGroundManager)
            which.GetComponent<oGroundMainManager>().ChangeCoords(new Vector2Int(j, _mapSize.x - 1 - i));
        // Update _mapGrid
    ...
    private void GroundSwap(GameObject which, Vector2Int newCoords)
    {
        ...
        _lastGroundSelected.GetComponent<oGroundMainManager>().GroundCoords = newCoords;
        which.GetComponent<oGroundMainManager>().GroundCoords = _lastGroundCoordsSelected;
        // Reset selection's color of the two Grounds
        _lastGroundSelected.GetComponent<oGroundMainManager>().ResetMat();
        which.GetComponent<oGroundMainManager>().ResetMat();
        // Reset selection
        ResetTempGrid();
        ResetLastSelected();
    ...
    private void CheckAroundGroundSelected(GameObject which, Vector2Int coords)
    {
        ...
        foreach (var dir in _directionsOne)
        {
            ...
            if (!MapGrid[newPos.x, newPos.y].GetComponent<oGroundMainManager>()) continue;
            // Check if ground CanBeMoved
            if (!MapGrid[newPos.x, newPos.y].GetComponent<oGroundMainManager>().CanBeMoved) continue;
            // Check if not actually selected -> Security
            if (MapGrid[newPos.x, newPos.y].GetComponent<oGroundMainManager>().IsSelected) continue;
            // It's good
            MapGrid[newPos.x, newPos.y].GetComponent<oGroundMainManager>().OnAroundedSelected();
            // Update temp grid
    ...
    private void CheckIfGoodAroundedPlayer(Vector2Int newPos)
    {
        ...
        if (map == null) return;
        // Check if has GroundManager
        if (!map.GetComponent<oGroundMainManager>()) return;
        // Check if ground CanBeMoved
        if (!map.GetComponent<oGroundMainManager>().CanBeMoved) return;
        // Make it arounded
        map.GetComponent<oGroundMainManager>().OnAroundedPlayer();
    ...
    private void ResetLastSelected()
    {
        ...
        {
            ground.GetComponent<oGroundMainManager>().ResetMat();
        }
    ...
    private void ResetLastSelectedPlayer()
    {
        ...
        {
            ground.GetComponent<oGroundMainManager>().ResetBaseMat();
        }
