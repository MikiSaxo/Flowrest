...
public class oEditorManager : MonoBehaviour
{
    ...
    public static oEditorManager Instance;
    ...
    private void InitObj(GameObject which, int x, int y)
    {
        ...
        which.GetComponent<oGroundEditorManager>().ChangeCoords(new Vector2Int(x, y));
        // Update _mapGrid
        _mapGrid[x, y] = which;
    }
    ...
    private void InitWater(GameObject which, char letter)
    {
        for (int i = 0; i < waterData.Length; i++)
        {
            if (waterData[i].WaterName == letter)
            {
                ...
                which.GetComponent<oWaterEditorManager>().ChangeWaterDir(waterData[i].DirectionsNSWE);
                break;
            }
        }
    }
    ...
    public void ChangeActivatedButton(GameObject button)
    {
        if (_lastButtonSelected != null)
            _lastButtonSelected.GetComponent<oGroundUIButton>().NeedActivateSelectedIcon(false);

        _lastButtonSelected = button;

        _lastButtonSelected.GetComponent<oGroundUIButton>().NeedActivateSelectedIcon(true);
    }
    ...
    private void ChangeGround()
    {
        ...
        if (_lastUIGroundSelected.GetComponent<GroundWhite>())
            _lastGroundSelected.GetComponent<oGroundEditorManager>()
            ...
            _lastGroundSelected.GetComponent<oGroundEditorManager>().EditorTransformTo(_lastUIGroundSelected,
                _lastButtonSelected.GetComponent<oEditorUIWaterButtonDir>().WhichDir);


        _lastGroundSelected.GetComponent<oGroundMainManager>().ResetMat();
        _lastGroundSelected.GetComponent<oGroundMainManager>().IsSelected = false;
